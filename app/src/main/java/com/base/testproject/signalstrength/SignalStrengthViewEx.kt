...
import com.base.testproject.R


class SignalStrengthViewEx @JvmOverloads constructor(
    context: Context,
    ...
    init {
        val typedArray = context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.SignalStrengthViewEx,
            defStyleAttr,
            R.style.Widget_SignalStrength
        )

        val themes = SignalStrength.Theme.values()
        val themeIndex = typedArray.getInt(R.styleable.SignalStrengthViewEx_signalStrengthTheme, 0)
            .coerceIn(0, themes.lastIndex)

        signalStrengthDrawable = SignalStrengthDrawable(context, themes[themeIndex])

        signalLevel =
            typedArray.getInt(R.styleable.SignalStrengthViewEx_signalStrengthLevel, signalLevel)
        color = typedArray.getColor(R.styleable.SignalStrengthViewEx_signalStrengthColor, color)
